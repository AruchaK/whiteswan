...
import type { Pillar, PillarKey } from '../lib/pillars'
...
import { addTask, useAddedTasks, type AddedTask } from '../lib/addedTasksStore'
import { initials } from '../lib/initials'

/* Pillars that surface an attention dot in the top pill bar. */
const ATTENTION_KEYS = new Set<PillarKey>(['personal'])

type SectionKey = 'tasks' | 'contacts' | 'notes'
...
function isPillarKey(value: string | undefined): value is PillarKey {
...
/* ── Top pillar switcher ── */
function PillarTabs({ activeKey }: { activeKey: PillarKey }) {
  return (
...
function Hero({ pillar, data }: { pillar: Pillar; data: PillarPlanningData }) {
...
/* ── Tasks panel ── */
function TaskRow({ task, done, onToggle }: { task: PlanningTask; done: boolean; onToggle: () => void }) {
  return (
...
function TasksPanel({ data, tasks, completedIds, onToggle, onAddTask }: TasksPanelProps) {
...
/* ── Contacts panel ── */
function ContactCard({ contact, pillar }: { contact: PlanningContact; pillar: Pillar }) {
  return (
...
function ContactsPanel({ data, pillar }: { data: PillarPlanningData; pillar: Pillar }) {
  return (
    <Card className="p-6">
...
/* ── Notes panel ── */
function NoteRow({ note, highlight = false, pillar }: { note: PlanningNote; highlight?: boolean; pillar: Pillar }) {
  return (
    <button
      type="button"
...
function NotesPanel({ data, pillar }: { data: PillarPlanningData; pillar: Pillar }) {
  return (
    <Card className="p-6">
...
export default function PillarPlanningPage() {
  ...
  const pillarKey = isPillarKey(pillarParam) ? pillarParam : undefined
  const pillar = pillarKey ? PILLARS_BY_KEY[pillarKey] : undefined
  const data = pillarKey ? PILLAR_DATA[pillarKey] : undefined
        ...
        <div className="max-w-300 mx-auto space-y-6 animate-fade-in">
          ...
          <Card className="p-8">
            <p className="text-[13px] text-espresso-600 italic">
              We couldn&rsquo;t find a pillar called &ldquo;{pillarParam}&rdquo;.
            </p>
          </Card>
        </div>
      </AppLayout>
  ...
  const handleAddTask = (task: AddedTask) => {
    addTask(task)
