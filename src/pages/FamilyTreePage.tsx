...
type MemberId = 'pisan' | 'somsri' | 'somchai' | 'pim' | 'nara' | 'ploy'
...
interface FamilyMember {
  ...
  badge: string | null
  reflection: string
  documents: ReadonlyArray<{ title: string; status: DocumentStatus; date: string }>
}
...
] as const satisfies readonly FamilyMember[]
...
function Avatar({ member, size = 60, selected = false }: { member: FamilyMember; size?: number; selected?: boolean }) {
  const gap = 2
...
const NODE_POSITIONS: Record<MemberId, { x: number; y: number }> = {
...
function TreeNode({ member, isSelected, onSelect }: TreeNodeProps) {
...
export default function FamilyTreePage() {
  const [selectedId, setSelectedId] = useState<MemberId>('somchai')
