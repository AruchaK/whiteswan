 ...
 *
 * Keyed by pillar key (see pillars.ts). Per-pillar page *content* — badges,
 * tasks, contacts, notes — still lives in pillarData.ts; this module owns only
...
} satisfies Record<PillarKey, PillarReadiness>

export type ReadinessEntry = PillarReadiness & { key: PillarKey }

/* Ordered list following the canonical PILLARS order, each entry carrying its key. */
export const READINESS_LIST: ReadinessEntry[] = PILLARS.map((p) => ({
  key: p.key,
  ...PILLAR_READINESS[p.key],
}))

/* "4 of 6 essentials complete" — derived, so it can never disagree with done/total. */
export function essentialsLabel(key: PillarKey): string {
